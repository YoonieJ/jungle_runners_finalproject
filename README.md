...
Stages are generated by `StageFactory` from a `StageDefinition`. Each stage creates a three-row `GridWorld` with deterministic content placement based on the stage number, so the same stage has consistent hazards and rewards. The current roll tables favor more arrows and meteors than earlier builds. Later stages increase pressure by changing rows more often, adding more dangerous obstacle clusters, and introducing homing projectiles.
...
The route graph is still a prototype. It currently includes an approach segment, optional canopy and ruins routes, and a temple gate merge. Stage 2 and Stage 3 offer branch choices, and Stage 3 triggers the Sunken Idol encounter near the end of the generated grid. Branch and merge markers are visible in top view and route-choice previews, but marker-only green boxes are hidden in front view so they do not look like pickups. A fuller graph and route-selection system is planned.
